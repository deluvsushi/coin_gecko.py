...
class CoinGecko:
	...
	def get_all_categories_with_market_data(self, order: str = None):
	...
	def get_exchanges_with_market_data(self):
		return get(
	...
	def get_market_index(self, market_id: str, index_id: str):
		return get(
			f"{self.api}/indexes/{market_id}/{index_id}",
			headers=self.headers).json()
	
	def get_indexes_with_market_data(self):
		return get(
			f"{self.api}/indexes/list",
			headers=self.headers).json()
	
	def get_all_derivative_tickers(self, include_tickers: str = "unexpired"):
		return get(
	...
	def get_derivative_exchange_data(
			self,
			exchange_id: str,
			include_tickers: str = "all"):
		return get(
			f"{self.api}/derivatives/exchanges/{exchange_id}?include_tickers={include_tickers}",
			headers=self.headers).json()
	
	def get_derivative_exchanges_list(self):
		return get(
			f"{self.api}/derivative/exchanges/list",
			headers=self.headers).json()
	
	def get_volume_chart_data(self, exchange_id: str, days: int):
		return get(
	...
	def get_global_crypto_defi_data(self):
		return get(
			f"{self.api}/global/decentralized_finance_defi",
			headers=self.headers).json()
	
	def get_public_companies_data(self, coin_id: str):
		return get(
